...
> - data ブロックは定数に近く、どの語彙ファイルやデータを使うかを指
> - model と train ブロックはまさにハイパーパラメータの集合です。モデル構造や正則化、最適化手法といったチューニング項目が並
...
> [!NOTE] 学習データ / 検証セット / テストセット / サンプリング率
> - 学習データ（train）
> 	- モデルのパラメータを更新するために使うデータ。ここで損失関数を最小化するよう重みを学習します。
> - 検証セット（validation）                                                                                                                                                             
> 	- **学習中にモデルの性能を客観的に測り、ハイパーパラメータ調整や早期終了の判断に使うデータ。** 学習には使わず、エポックごとなどに評価して「これ以上よくならない」と判断したら学習を止めます。
> - テストセット（test）                                                                                                                                                                 
> 	- 最終的な性能を報告するために使うデータ。学習やハイパーパラメータ調整には一切使わず、学習完了後に一度だけ評価して汎化性能を確認します。


> [!NOTE] 学習回数(エポック)、バッチ、イテレーション
> - **バッチサイズ**
> 	- 1回のパラメータ更新で処理する訓練データの件数。全データをこのサイズごとに区切った集合がミニバッチ。
> - **イテレーション**
> 	- ミニバッチを1つ処理してパラメータを更新する操作。1エポック中のイテレーション回数は「訓練データ総数÷バッチサイズ)
> - **エポック数**
