...
$$
ここで、 $\mathbf{w} \in \mathbb{R}^{2D}$ は学習の対象となるパラメータ。

$\mathbf{w}$ と $\mathbf{q}$ の相互作用によって計算される注意 → **内容に基づく注意 (content-based attention)**
$\mathbf{q}$ のみを使って $a = \text{softmax} (\mathbf{Wq})$ のように $\mathbf{z}$ には依存しない形で注意を求める方法 → **位置に基づく注意 (location-based attention)**

いずれの場合でも注意はネットワーク内で得られた $\mathbf{z}_i$ にネットワーク内の別の場所で作り出された重み $a_i$ を掛け合わせる計算を行っている
**→ ネットワーク内部の信号同士で積を求めており、この点でゲート機構と同じ。**
