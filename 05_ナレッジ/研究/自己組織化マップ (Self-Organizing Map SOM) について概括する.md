...
まず潜在空間 $Z$ を閉区間とします（下のpython実装ではその領域については $Z=[−1,1]L$ としてあります）．  
次に潜在空間全体を $K$ 個の代表点（これをノードと呼ぶ）で離散表現します． $k$ 番ノードの座標を $ζk∈Z$ とおきます．  
これらの制約によって，写像 $f$ 全体は各ノード座標 ${ζk}k=1K$ からの $K$ 個の写像 $yk=f(ζk)$ によって離散表現できます（この $yk$ を参照ベクトルと呼ぶ）．  
